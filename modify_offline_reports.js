...
files.forEach(file => {
  ...
  // Add missing state variables just in case
  ...
  // Replace fetchReports block completely
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const res = await axios.get(\`http://localhost:3000/api/\${file.includes('Tanaka') ? 'marketing-offline-tanaka' : 'marketing-offline'}/orders\`, { headers: { Authorization: \`Bearer \${token}\` } });
