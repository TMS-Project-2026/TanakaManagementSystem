...
exports.createInvoice = (req, res) => {
    ...
    const sql = `
        INSERT INTO invoice (
            ...
            note, file_supporting, materai, ttd, nama_accounting, penanggung_jawab, jabatan, status, term_of_payment, ongkos_kirim
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const values = [
        ...
        note, file_supporting ? JSON.stringify(file_supporting) : null, materai ? 1 : 0, ttd ? 1 : 0, nama_accounting, penanggung_jawab, jabatan, status || 'Draft', term_of_payment, ongkos_kirim || 0
    ];
...
exports.updateInvoice = (req, res) => {
    ...
    db.query("SELECT status FROM invoice WHERE id = ?", [id], (errCheck, resCheck) => {
        if (errCheck) return res.status(500).json({ status: "error", message: errCheck.message });
        if (resCheck.length === 0) return res.status(404).json({ status: "error", message: "Invoice tidak ditemukan!" });
        
        const oldStatus = resCheck[0].status;

        db.query(sql, values, async (err, result) => {
            if (err) return res.status(500).json({ status: "error", message: err.message });

            // JEMBATAN KE GUDANG DAN PRODUKSI
            // Jika status invoice berubah dari Draft menjadi 'Terbit' atau 'Lunas', potong stok.
            if ((fields.status === 'Terbit' || fields.status === 'Lunas') && oldStatus === 'Draft') {
                try {
