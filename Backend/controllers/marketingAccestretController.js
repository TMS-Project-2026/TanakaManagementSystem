...
exports.getDashboard = (req, res) => {
    ...
    const queries = {
        revenueToday: `SELECT SUM(actual) as revenue FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' AND order_date = ?`,
        ordersToday: `SELECT COUNT(id) as total_orders FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' AND order_date = ?`,
        monthlySummary: `SELECT 
                         ...
                         WHERE type = 'online' AND branch = 'Accestret' 
                         AND order_date >= ?`,
        topProducts: `SELECT product_name, SUM(qty) as total_qty, SUM(actual) as total_sales FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' GROUP BY product_name ORDER BY total_qty DESC LIMIT 5`,
        salesChart: `SELECT order_date, SUM(actual) as revenue, COUNT(id) as orders FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' AND order_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) GROUP BY order_date ORDER BY order_date ASC`,
        topToko: `SELECT akun_toko, SUM(actual) as total_revenue, COUNT(id) as total_orders, SUM(qty) as total_qty FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' AND akun_toko IS NOT NULL AND akun_toko != '' AND akun_toko != '-' GROUP BY akun_toko ORDER BY total_revenue DESC`
    };
...
exports.getOrders = (req, res) => {
    const sql = `SELECT * FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' ORDER BY order_date DESC`;
...
exports.importShopee = async (req, res) => {
    ...
    try {
        ...
        for (const order of orders) {
            ...
            // Check stock in inventory for branch 'Accestret'
            let stokResult = [];
            if (order.stok_id) {
                [stokResult] = await promiseDb.query(
                    "SELECT id, jumlah FROM stok WHERE id = ? AND cabang_id = 'Accestret'",
                    [order.stok_id]
                );
            } else {
                [stokResult] = await promiseDb.query(
                    "SELECT id, jumlah FROM stok WHERE nama_barang = ? AND cabang_id = 'Accestret'",
                    [productName]
                );
            }
            ...
            if (stokResult.length === 0) {
                ...
                return res.status(404).json({ message: `Barang tidak ditemukan di gudang Accestret: ${productName}. Tidak dapat membuat pesanan.` });
            }
        ...
        await promiseDb.query("ROLLBACK");
        console.error("Error import Accestret:", error);
        res.status(500).json({ message: "Gagal menyimpan pesanan: " + error.message });
    }
};

// GET INVENTORY (Stok cabang Accestret)
exports.getInventory = (req, res) => {
    const sql = `SELECT id, nama_barang as product_name, jumlah as stock_qty, kategori, minimum_stok FROM stok WHERE cabang_id = 'Accestret' ORDER BY nama_barang ASC`;
    db.query(sql, [], (err, results) => {
        if (err) {
...
exports.getReports = (req, res) => {
    const queries = {
        harian: `SELECT order_date as date, SUM(actual) as revenue, COUNT(id) as orders FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' GROUP BY order_date ORDER BY order_date DESC LIMIT 30`,
        bulanan: `SELECT DATE_FORMAT(order_date, '%Y-%m') as month, SUM(actual) as revenue, COUNT(id) as orders FROM marketing_orders_online WHERE type = 'online' AND branch = 'Accestret' GROUP BY DATE_FORMAT(order_date, '%Y-%m') ORDER BY month DESC LIMIT 12`,
    };

    let reports = { harian: [], bulanan: [] };
...
exports.ajukanKeFinance = (req, res) => {
    ...
    db.query("SELECT * FROM marketing_orders_online WHERE id = ? AND branch = 'Accestret'", [id], (err, results) => {
        ...
        const sqlApproval = `INSERT INTO approvals (tipe, keterangan, nominal, diajukan_oleh, status, tanggal_pengajuan, reference_id) 
                             VALUES ('quotation_to_invoice', ?, ?, 'Marketing Accestret', 'pending', CURRENT_TIMESTAMP, ?)`;
...
exports.getTargets = (req, res) => {
    const sql = `SELECT account_name, target_type, target_value FROM marketing_targets WHERE branch = 'Accestret' ORDER BY target_type, account_name`;
    db.query(sql, [], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
...
exports.updateTarget = (req, res) => {
    ...
    db.query(sql, [account_name, target_type, roundedValue], (err) => {
...
// GET PROMO STOCK (Dead Stock >= 60 days untuk Accestret)
        ...
        WHERE
            ...
            AND s.nama_barang NOT IN (
                ...
                WHERE order_date >= DATE_SUB(CURDATE(), INTERVAL 60 DAY)
                  AND branch = 'Accestret'
            )
        ORDER BY hari_mengendap DESC
    `;
    db.query(sql, [], (err, results) => {
        if (err) {
            console.error('Error get promo stock Accestret:', err);
            return res.status(500).json({ error: err.message });
        }
        res.json(results);
    });
