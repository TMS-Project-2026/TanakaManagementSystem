...
exports.getAllBarangMasuk = async (req, res) => {
    try {
        ...
        const sql = `
            SELECT bm.*, bm.jumlah_reject, s.kode_produk, s.nama_brand, s.nama_barang, s.bahan, s.cabang_id, s.ukuran, s.kategori, s.minimum_stok 
    ...
    try {
        ...
        if (items && Array.isArray(items)) {
            ...
            for (const item of items) {
                const qtyMasuk = Number(item.jumlah) || 0;
                const qtyReject = Number(item.jumlah_reject) || 0;
                ...
                if (existing.length > 0) {
                    ...
                    // Stok bertambah sebesar jumlah masuk bersih (yang diinput user)
                    await promiseDb.query(
                        "UPDATE stok SET jumlah = jumlah + ?, minimum_stok = ?, nama_brand = ?, kategori = ?, kode_rak = ? WHERE id = ?",
                        [qtyMasuk, minStok, nama_brand || null, kategori, kode_rak || null, stockId]
                    );
                } else {
                    ...
                    const [insertRes] = await promiseDb.query(
                        ...
                        [kodeProduk, nama_brand || null, nama_barang, bahanProduk, qtyMasuk, kategori, cabang_id, minStok, kode_rak || null, size]
                    );
                    stockId = insertRes.insertId;
                }

                // Masukkan transaksi (simpan masuk bersih + reject untuk laporan)
                await promiseDb.query(
                    "INSERT INTO barang_masuk (transaksi_id, barang_id, jumlah, jumlah_reject, tanggal, supplier) VALUES (?, ?, ?, ?, ?, ?)",
                    [trxId, stockId, qtyMasuk, qtyReject, tanggal, supplier || null]
                );
            }
        ...
        const sqlInsert = "INSERT INTO barang_masuk (transaksi_id, barang_id, jumlah, jumlah_reject, tanggal, supplier) VALUES (?, ?, ?, ?, ?, ?)";
        await promiseDb.query(sqlInsert, [trxId, barang_id, jumlah, 0, tanggal, supplier || null]);
...
exports.updateBarangMasuk = async (req, res) => {
    ...
    try {
        ...
        // 1. Revert old stock (kembalikan stok berdasarkan jumlah bersih lama)
        const [oldItems] = await promiseDb.query("SELECT * FROM barang_masuk WHERE transaksi_id = ?", [transaksi_id]);
        for (const old of oldItems) {
            const oldBersih = old.jumlah || 0;
            await promiseDb.query("UPDATE stok SET jumlah = jumlah - ? WHERE id = ?", [oldBersih, old.barang_id]);
        }
        ...
        if (items && Array.isArray(items)) {
            for (const item of items) {
                ...
                await promiseDb.query(
                    "INSERT INTO barang_masuk (transaksi_id, barang_id, jumlah, jumlah_reject, tanggal, supplier) VALUES (?, ?, ?, ?, ?, ?)",
                    [transaksi_id, stockId, qtyMasuk, qtyReject, tanggal, supplier || null]
                );
            }
        } else if (req.body.barang_id && req.body.jumlah) {
             // Fallback ke format lama untuk update
             const sqlInsert = "INSERT INTO barang_masuk (transaksi_id, barang_id, jumlah, jumlah_reject, tanggal, supplier) VALUES (?, ?, ?, ?, ?, ?)";
             await promiseDb.query(sqlInsert, [transaksi_id, req.body.barang_id, req.body.jumlah, 0, tanggal, supplier || null]);
...
exports.deleteBarangMasuk = async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            // Saat hapus, kembalikan stok sebesar jumlah bersih
            const qtyBersih = item.jumlah || 0;
            await promiseDb.query("UPDATE stok SET jumlah = jumlah - ? WHERE id = ?", [qtyBersih, item.barang_id]);
        }
