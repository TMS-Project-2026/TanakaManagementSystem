...
// We must get the SECOND occurrence of "{activeTab === 'reports' && ("
const searchStr = "{activeTab === 'reports' && (";
const firstIndexOnline = onlineFile.indexOf(searchStr);
const uiStartIndex = onlineFile.indexOf(searchStr, firstIndexOnline + 1);
const endMarkerOnline = "{/* TAB: PROMO ONLINE */}";
const uiEndIndex = onlineFile.indexOf(endMarkerOnline, uiStartIndex);
...
`;
    ...
    const firstIndexOff = content.indexOf(searchStr);
    const offStartIndex = content.indexOf(searchStr, firstIndexOff + 1);
