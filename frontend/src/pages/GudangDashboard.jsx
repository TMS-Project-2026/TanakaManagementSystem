                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 bg-[#f8fafc]">
                    <div className="max-w-[1600px] mx-auto space-y-5 animate-in fade-in slide-in-from-bottom-6 duration-700">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            {[
                                ...
                                <div key={index} className="bg-white rounded-[12px] p-3.5 shadow-sm border border-gray-100 border-l-[4px] border-l-[#990000] flex flex-col justify-center transition-all hover:-translate-y-1 hover:shadow-md min-h-[75px]">
                                    ...
                                    <h3 className="text-lg font-black text-gray-900 leading-tight truncate">{card.value}</h3>
                        ...
                        <div className={`grid grid-cols-1 ${isOwner ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-4`}>
                            ...
                            <div className="lg:col-span-2 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-xs font-bold text-gray-800 flex items-center gap-2">
                                        <TrendingUp className="text-[#990000]" size={14} /> Pergerakan Barang (7 Hari)
                                    </h3>
                                </div>
                                    ...
                                    {data.chartData && data.chartData.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={data.chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                                                ...
                                                <RechartsTooltip contentStyle={{ borderRadius: '12px', border: 'none', fontSize: '11px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                                                <Legend wrapperStyle={{ fontSize: '9px', fontWeight: 'bold', paddingTop: '5px' }} />
                                                <Line type="monotone" dataKey="masuk" name="Masuk" stroke="#990000" strokeWidth={2.5} dot={{ r: 2 }} />
                                                <Line type="monotone" dataKey="keluar" name="Keluar" stroke="#ff4d4d" strokeWidth={2.5} dot={{ r: 2 }} />
                                            </LineChart>
                                        </ResponsiveContainer>
                                    ) : (
                                        <div className="h-full flex items-center justify-center text-[10px] text-gray-400 font-medium">Belum ada data</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                                <h3 className="text-xs font-bold text-gray-800 mb-2.5 flex items-center gap-2">
                                    <AlertTriangle className="text-orange-500" size={14} /> Barang Hampir Habis
                                </h3>
                                <div className="space-y-2 flex-1 overflow-y-auto max-h-[140px] pr-1 custom-scrollbar">
                                    {data.hampirHabis && data.hampirHabis.length > 0 ? (
                                        ...
                                        <div className="text-center text-[9px] text-gray-400 font-bold italic py-6">Semua stok aman!</div>
                                    )}
                                </div>
                            </div>
                            ...
                            {isOwner && (
                                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col border-t-[3px] border-t-blue-500">
                                    <h3 className="text-xs font-bold text-gray-800 mb-2.5 flex items-center gap-2">
                                        <CheckCircle className="text-blue-500" size={14} /> Approval Stok
                                    </h3>
                                    <div className="space-y-2.5 flex-1 overflow-y-auto max-h-[140px] pr-1 custom-scrollbar">
                                        ...
                                        )) : <div className="text-center text-[9px] text-gray-400 font-bold italic py-6">Tidak ada permintaan pending</div>}
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pb-4">
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                                <h3 className="text-xs font-bold text-gray-800 mb-2.5 flex items-center gap-2">
                                    <TrendingUp className="text-green-500" size={14} /> Fast Moving (30 Hari)
                                </h3>
                                        ...
                                        data.fastMoving.map((item, idx) => (
                                            <div key={idx} className="flex flex-col gap-1 border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                                                ...
                                                <div className="flex items-center gap-2 pl-7">
                                                    <div className="flex-1 bg-gray-100 rounded-full h-1 overflow-hidden">
                                                        <div
                                                            className="h-1 rounded-full transition-all duration-700 bg-emerald-500"
                                                            style={{ width: `${Math.min(100, ((item.total_terjual || 0) / (data.fastMoving[0]?.total_terjual || 1)) * 100)}%` }}
                                                        />
                                                    </div>
                                                    <span className="text-[9px] font-black text-emerald-600 shrink-0">{item.total_terjual} Pcs</span>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="text-center text-[9px] text-gray-400 font-bold italic py-6">Belum ada data penjualan</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                                <div className="flex items-center justify-between mb-2.5">
                                    <h3 className="text-xs font-bold text-gray-800 flex items-center gap-2">
                                        <AlertTriangle className="text-red-600" size={14} /> Dead Stock ({'>'}60 Hari)
                                    </h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 flex-1 overflow-y-auto max-h-[120px] pr-1 custom-scrollbar">
                                    {data.deadStock && data.deadStock.length > 0 ? (
                                        data.deadStock.map((item, idx) => (
                                            <div key={idx} className="bg-gray-50/50 hover:bg-gray-50 p-2.5 rounded-xl border border-gray-100 flex justify-between items-center transition-all">
                                                <div className="min-w-0 flex-1 pr-2">
                                                    <p className="font-bold text-gray-800 text-xs truncate">{item.nama_barang} {item.ukuran ? `- ${item.ukuran}` : ''}</p>
                                                    <p className="text-[9px] text-red-500 font-bold mt-0.5">⚠️ Mengendap &gt; 2 Bulan</p>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <span className="inline-block bg-red-50 border border-red-100 text-red-600 text-[9px] font-black px-2 py-0.5 rounded-md">{item.jumlah} Pcs</span>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="text-center text-[9px] text-gray-400 font-bold italic py-6 col-span-full">Semua stok berputar dengan baik!</div>
                                    )}
                                </div>
                            </div>
                        </div>
